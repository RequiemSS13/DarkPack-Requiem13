...
/proc/init_vampire_clan_list()
	...
	for (var/datum/vampire_clan/clan_type as anything in valid_subtypesof(/datum/vampire_clan/vtr))
		clan_list[initial(clan_type.name)] = clan_type
	clan_list = sort_list(clan_list)
	return clan_list

/// Associative list of Clan typepaths to singletons
GLOBAL_LIST_INIT_TYPED(vampire_clans, /datum/vampire_clan/vtr, init_vampire_clans())
